...
void motorForward(int motorNum, int motorPower)
{ //Makes Motor motorNum go forwards at a power of motorPower
  if (motorNum >= 0 and motorNum <= 5)
  {
    if (motorPower < 0)
    { //Lowest power possible = 0.
      motorPower = 0;
    }
    if (motorPower > 100)
    { //Highest power possible = 100.
      motorPower = 100;
    }
...
void motorBackward(int motorNum, int motorPower)
{ //Makes Motor motorNum go backwards at a power of motorPower
  if (motorNum >= 0 and motorNum <= 5)
  {
    if (motorPower < 0)
    {
...
void setPWMpin(int portNum, int power)
{ //PortNum is the Sensor port used, pwm is the power of the output (between 0-100)
...
int readAnalogSensorData(int portNum)
{ //PortNum is the Sensor port used
  if (portNum >= 0 and portNum <= 3)
  {
    int sensorData = -1;
    switch (portNum)
