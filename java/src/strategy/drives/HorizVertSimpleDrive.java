...
public class HorizVertSimpleDrive implements DriveInterface {
  ...
  public String robotInPath(
      PolarCoordinate[] robots,
      double ourRadius,
      double ourAngle,
      double targetRadius,
      double targetAngle) {
    ...
    for (PolarCoordinate robot : robots) {
      ...
      } else if (robot.getAngle() < Math.max(ourAngle, targetAngle)
          && robot.getAngle() > Math.min(ourAngle, targetAngle)) {
  ...
  public BallTrackState getBallTrackState() {
    ...
    if (angleDiffAbs < angleThreshold2
      ...
      if (robotInPath(
              players,
              diag4PolarCoords.getRadius(),
              diag4PolarCoords.getAngle(),
              targetRadius,
              targetAngle)
          == "In front") {
  ...
  public void move(RobotPort port) {
    ...
    ((FourWheelHolonomicRobotPort) port)
        .fourWheelHolonomicMotion(
            totalPowerDrive[0], totalPowerDrive[1], totalPowerDrive[2], totalPowerDrive[3]);
  }
