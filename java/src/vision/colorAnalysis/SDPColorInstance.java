...
// RK: Modified to handle automatic setting saving and region info in the title
public class SDPColorInstance extends JFrame
        implements ActionListener, ChangeListener, PreviewSelectionListener, SaveLoadCapable {
    ...
    public SDPColorInstance(String name, int region, Color referenceColor, SDPColor sdpColor) {
        super();
        this.name = name;
        this.region = region;
        this.setupGUI();
    ...
    public void setRegionInTitle(int regionID) {
    ...
    private void refreshColors() {
        this.referenceColor =
                new Color(
                        Color.HSBtoRGB(
                                (this.minHue + this.maxHue) / 2,
                                (this.maxSaturation + this.minSaturation) / 2,
                                (this.minBrightness + this.maxBrightness) / 2));
        this.negatedColor =
                new Color(
                        255 - this.referenceColor.getRed(),
                        255 - this.referenceColor.getGreen(),
                        255 - this.referenceColor.getBlue());
    }
    ...
    public void myRepaint() {
        // Update title
        String infix = " (IN REGION ";
        if (region < 0 || region > 14) infix = " (UNKNOWN REGION";
        else infix += region;
        this.setTitle(name + infix + ")");
        ...
        this.minHuePanel.setBackground(
                Color.getHSBColor(
                        this.minHue,
                        (this.minSaturation + this.maxSaturation) / 2,
                        (this.minBrightness + this.maxBrightness) / 2));
        this.maxHuePanel.setBackground(
                Color.getHSBColor(
                        this.maxHue,
                        (this.minSaturation + this.maxSaturation) / 2,
                        (this.minBrightness + this.maxBrightness) / 2));
        this.maxSaturationPanel.setBackground(
                Color.getHSBColor(
                        (this.minHue + this.maxHue) / 2,
                        this.maxSaturation,
                        (this.minBrightness + this.maxBrightness) / 2));
        this.minSaturationPanel.setBackground(
                Color.getHSBColor(
                        (this.minHue + this.maxHue) / 2,
                        this.minSaturation,
                        (this.minBrightness + this.maxBrightness) / 2));
        this.maxBrightnessPanel.setBackground(
                Color.getHSBColor(
                        (this.minHue + this.maxHue) / 2,
                        (this.maxSaturation + this.maxSaturation) / 2,
                        this.maxBrightness));
        this.minBrightnessPanel.setBackground(
                Color.getHSBColor(
                        (this.minHue + this.maxHue) / 2,
                        (this.minSaturation + this.maxSaturation) / 2,
                        this.minBrightness));

        // Save settings to given region file, if we're in it
        if (region >= 0 || region <= 14) {
            try {
                SettingsManager.saveSettingsToFile(Constants.settingsFilePath(region));
            }
            catch (Exception e) { }
        }
    ...
    public void previewClickHandler(ColoredPoint coloredPoint) {
        if (this.calibrating && this.isVisible()) {
            float[] f =
                    Color.RGBtoHSB(
                            coloredPoint.color.getRed(),
                            coloredPoint.color.getGreen(),
                            coloredPoint.color.getBlue(),
                            null);
