...
public class PotentialRobot {
  ...
  public DetectedRobot newSpot(Spot s) {
    ...
    if (determined != null) {
      angle =
          VectorGeometry.angle(
                  determined.get(0).x - this.baseSpot.x, determined.get(0).y - this.baseSpot.y)
              - 2.53073;
      return new DetectedRobot(
          robotType, new DirectedPoint((int) this.baseSpot.x, (int) this.baseSpot.y, angle));
    }
