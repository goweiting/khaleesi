...
public class Distortion extends JPanel
    implements SaveLoadCapable,
        ...
        ChangeListener,
        NextSpotsListener,
        DistortionPreviewClickListener {
    ...
    if (this.savedImage != null) {
      ...
      for (int y = 0; y < this.savedImage.getHeight(); y++) {
        for (int x = 0; x < this.savedImage.getWidth(); x++) {
          ...
          if (pg.x < this.savedImage.getWidth()
              && pg.x >= 0
              && pg.y < this.savedImage.getHeight()
              && pg.y >= 0) {
      ...
      g.drawRect(
          this.topLeftPoint.x,
          this.topLeftPoint.y,
          this.bottomRightPoint.x - this.topLeftPoint.x,
          this.bottomRightPoint.y - this.topLeftPoint.y);
    }
  }
  ...
  private void normalizePoint(VectorGeometry pg) {
    ...
    pg.x =
        (pg.x - this.topLeftPoint.x)
                * Constants.PITCH_WIDTH
                / (this.bottomRightPoint.x - this.topLeftPoint.x)
            - Constants.PITCH_WIDTH / 2;
    pg.y =
        -((pg.y - this.topLeftPoint.y)
                * Constants.PITCH_HEIGHT
                / (this.bottomRightPoint.y - this.topLeftPoint.y)
            - Constants.PITCH_HEIGHT / 2);
  }
