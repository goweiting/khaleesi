...
public class RecursiveSpotAnalysis extends SpotAnalysisBase {
  ...
  private void processPixel(
      int x, int y, SDPColorInstance sdpColorInstance, XYCumulativeAverage average, int maxDepth) {
    if (maxDepth <= 0
        || x < 0
        || x >= Constants.INPUT_WIDTH
        || y < 0
        || y >= Constants.INPUT_HEIGHT) return;
    int i = getIndex(x, y);
        ...
        for (int x = 0; x < Constants.INPUT_WIDTH; x++) {
          ...
          if (average.getCount() > 5) {
            spots
                ...
                .add(
                    new Spot(
                        average.getXAverage(), average.getYAverage(), average.getCount(), color));
          }
          average.reset();
        }
      }
