 ...
 *
 * <p>VERY DANGEROUS CLASS! Sometimes fails misserably, this is the predecessor of the
 * RecursiveSpotAnalysis class
 */
  ...
  public void nextFrame(BufferedImage bi, long time) {
    ...
    for (RegionFinder regionFinder : this.regionFinders.values()) {
      for (int i = 0; i < Constants.INPUT_WIDTH * Constants.INPUT_HEIGHT; i++) {
        if (isChanged(i)) {
          regionFinder.nextPixel(
              hsv[3 * i],
              hsv[3 * i + 1],
              hsv[3 * i + 2],
              i % Constants.INPUT_WIDTH,
              i / Constants.INPUT_WIDTH);
        }
      }
    }
        ...
        if (spot != null) {
          if (g != null)
            g.drawRect(
                potentialSpot.x1,
                potentialSpot.y1,
                (potentialSpot.x2 - potentialSpot.x1),
                (potentialSpot.y2 - potentialSpot.y1));
