  ...
  public Video(CaptureCallback cc, String port, int channel, short standard) {
    ...
    try {
      ...
      ImageFormat imageFormat =
          videoDevice.getDeviceInfo().getFormatList().getJPEGEncodableFormat(0);
      frameGrabber =
          videoDevice.getJPEGFrameGrabber(width, height, channel, standard, 100, imageFormat);
      frameGrabber.setCaptureCallback(cc);
      width = frameGrabber.getWidth();
      height = frameGrabber.getHeight();
    } catch (V4L4JException e1) {
      SDPConsole.message(
          "The video device could not be created. Try changing the channel to a different number.",
          (Component) cc);
      // cleanup and exit
      destroyCamera();
      return;
    ...
    try {
      ...
      SDPConsole.message(
          "The video capture could not be started. Try changing the channel to a different number.",
          (Component) cc);
      return;
    }
  }
