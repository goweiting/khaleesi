...
/** Created by Rado Kirilchev The idea here is that we patrol the general area of our goal. */
public class PatrolGoal extends ActionBase {

  private static final int ourGoalX =
      -Constants.PITCH_WIDTH / 2 + 40; // A little bit in front of the goal
  private static final ConstantPoint upperLimit =
      new ConstantPoint(ourGoalX, Constants.PITCH_HEIGHT / 2);
  private static final ConstantPoint lowerLimit =
      new ConstantPoint(ourGoalX, -Constants.PITCH_HEIGHT / 2);
  ...
  public void update() {
    ...
    double dist =
        VectorGeometry.distance(
            us.location, new VectorGeometry(curDestination.getX(), curDestination.getY()));
    if (dist < 30) {
      // Change destination.
