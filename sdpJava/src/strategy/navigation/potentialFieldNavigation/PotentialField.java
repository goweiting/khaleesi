...
public class PotentialField {
  ...
  public double getPotentialAtPoint(VectorGeometry point) {
    double d = 0;
    for (PotentialSourceInterface source : this.sources) {
      d = d + source.getPotentialAtPoint(source.getRelativePoint(point));
    }
    return d;
  }
  ...
  public VectorGeometry getForce(VectorGeometry point) {
    ...
    for (PotentialSourceInterface source : this.sources) {
      force.add(source.getForce(point));
    }
    return force;
  }
  ...
  public PotentialField addWalls() {
    ...
    this.addSource(new LineSource(new VectorGeometry(halfWidth, halfHeight),
        new VectorGeometry(halfWidth, -halfHeight), false, WALL_FIELD_FORMULA).setSize(WALL_SIZE)
        .setYFactor(WALL_Y_FACTOR));
    this.addSource(new LineSource(new VectorGeometry(halfWidth, -halfHeight),
        new VectorGeometry(-halfWidth, -halfHeight), false, WALL_FIELD_FORMULA).setSize(WALL_SIZE)
        .setYFactor(WALL_Y_FACTOR));
    this.addSource(new LineSource(new VectorGeometry(-halfWidth, -halfHeight),
...
//        this.addSource(new LineSource(new VectorGeometry(halfWidth - 60, 60), new VectorGeometry(halfWidth - 60, -60), false, WALL_FIELD_FORMULA).setSize(WALL_SIZE).setYFactor(WALL_Y_FACTOR));
    this.addSource(new LineSource(new VectorGeometry(halfWidth - 30, 30),
        new VectorGeometry(halfWidth - 30, -30), false, WALL_FIELD_FORMULA).setSize(WALL_SIZE + 15)
        .setYFactor(WALL_Y_FACTOR));
    return this;
  }
...
//        this.addSource(new LineSource(new VectorGeometry(-halfWidth + 60, 60), new VectorGeometry(-halfWidth + 60, -60), false, WALL_FIELD_FORMULA).setSize(WALL_SIZE).setYFactor(WALL_Y_FACTOR));
  ...
  public void setObstacles(LinkedList<Obstacle> obstacles) {
    for (Obstacle o : obstacles) {
      this.addSource(new PointSource(new VectorGeometry(o.x, o.y), false, FieldFormula.E_TO_MINUS_X)
          .setSize(30));
    }
  }
  ...
  public static PotentialField worldToPotentialField(DynamicWorld world) {
    ...
    for (Robot r : world.getRobots()) {
      if (r != null && r.type != RobotType.FRIEND_2) {
        field.addSource(
            new PointSource(r.location.clone(), false, FieldFormula.ONE_OVER_X2).setSize(35));
      }
    }
    ...
    if (friend != null && WorldTools.isPointInFriendDefenceArea(friend.location) && us != null
        && !WorldTools.isPointInFriendDefenceArea(us.location)) {
