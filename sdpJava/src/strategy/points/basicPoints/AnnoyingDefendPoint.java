...
public class AnnoyingDefendPoint extends DynamicPointBase {
  ...
  public void recalculate() {
    ...
    if (ball != null && ball.velocity.length() > 0.2) {
      ...
      if (foe1 != null) {
        closest =
            VectorGeometry.intersectionWithFiniteLine(
                foe1.location,
                VectorGeometry.fromAngular(foe1.location.direction, 10, null),
                lower,
                upper);
      }
      ...
      if (foe2 != null) {
        ...
        VectorGeometry foe2diff =
            VectorGeometry.fromTo(foe2.location, new VectorGeometry(-Constants.PITCH_WIDTH, 0));

        VectorGeometry foe1angular = new VectorGeometry(0, 0);
        VectorGeometry foe1diff = new VectorGeometry(0, 0);
        if (foe1 != null) {
          foe1angular = VectorGeometry.fromAngular(foe1.location.direction, 10, null);
          foe1diff =
              VectorGeometry.fromTo(foe1.location, new VectorGeometry(-Constants.PITCH_WIDTH, 0));
        }

        if (foe1 == null
            || VectorGeometry.angle(foe2angular, foe2diff)
                < VectorGeometry.angle(foe1angular, foe1diff)) {
          closest =
              VectorGeometry.intersectionWithFiniteLine(
                  foe2.location,
                  VectorGeometry.fromAngular(foe2.location.direction, 10, null),
                  lower,
                  upper);
        }
      }
