...
public class ProbabilisticRobot {
    ...
    public double getProbability(RobotType type, double distance) {
        if (team != null) {
            if (team == RobotColorSettings.FRIEND_COLOR && (type == RobotType.FOE_1 || type == RobotType.FOE_2))
                return 0;
            if (team == RobotColorSettings.FOE_COLOR && (type == RobotType.FRIEND_1 || type == RobotType.FRIEND_2))
                return 0;
        }
...
//        if(!morePink && (type==RobotType.FOE_2 || type==RobotType.FRIEND_2)) return 0;
        ...
        int teamBonus = 0;
        if ((team == RobotColorSettings.FRIEND_COLOR) && (type == RobotType.FRIEND_1 || type == RobotType.FRIEND_2))
            teamBonus = teamBonus + 40;
        if ((team == RobotColorSettings.FOE_COLOR) && (type == RobotType.FOE_1 || type == RobotType.FOE_2))
            teamBonus = teamBonus + 40;
