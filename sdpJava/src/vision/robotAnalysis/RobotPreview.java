...
public class RobotPreview extends JFrame implements DistortionListener, DynamicWorldListener {
  ...
  private RobotPreview() {
    ...
    this.image =
        new BufferedImage(
            Constants.PITCH_WIDTH + 2 * bounds,
            Constants.PITCH_HEIGHT + 2 * bounds,
  ...
  private void resetImage() {
    ...
    g.drawLine(
        bounds + Constants.PITCH_WIDTH / 2,
        Constants.PITCH_HEIGHT + bounds,
        bounds + Constants.PITCH_WIDTH / 2,
        bounds);
    g.drawLine(
        bounds,
        Constants.PITCH_HEIGHT / 2 + bounds,
        Constants.PITCH_WIDTH + bounds,
        Constants.PITCH_HEIGHT / 2 + bounds);
  }
  ...
  public void nextUndistortedSpots(HashMap<SDPColor, ArrayList<Spot>> spots, long time) {
    ...
    for (SDPColor color : spots.keySet()) {
      for (Spot spot : spots.get(color)) {
        ...
        g.drawArc(
            (int) spot.x - 2 + Constants.PITCH_WIDTH / 2 + bounds,
            -(int) spot.y - 2 + Constants.PITCH_HEIGHT / 2 + bounds,
            4,
            4,
            0,
            360);
  ...
  public void drawArc(int x, int y, int radius, Color color) {
    ...
    if (g != null) {
      g.setColor(color);
      g.drawArc(
          x - radius / 2 + Constants.PITCH_WIDTH / 2 + bounds,
          -y - radius / 2 + Constants.PITCH_HEIGHT / 2 + bounds,
          radius,
          radius,
          0,
          360);
  ...
  public void drawRect(int x, int y, int width, int height, Color color) {
    ...
    if (g != null) {
      g.setColor(color);
      g.drawRect(
          x + Constants.PITCH_WIDTH / 2 + bounds - height / 2,
          -y + Constants.PITCH_HEIGHT / 2 + bounds - height / 2,
          width,
          height);
    }
  }
  ...
  public void drawLine(int x1, int y1, int x2, int y2, Color color) {
    ...
    if (g != null) {
      g.setColor(color);
      g.drawLine(
          x1 + Constants.PITCH_WIDTH / 2 + bounds,
          -y1 + Constants.PITCH_HEIGHT / 2 + bounds,
          x2 + Constants.PITCH_WIDTH / 2 + bounds,
          -y2 + Constants.PITCH_HEIGHT / 2 + bounds);
    }
  }
  ...
  public void drawString(String string, int x, int y) {
    ...
    if (g != null) {
      g.setColor(Color.WHITE);
      g.drawString(
          string, x + Constants.PITCH_WIDTH / 2 + bounds, -y + Constants.PITCH_HEIGHT / 2 + bounds);
    }
  }
    ...
    for (RobotType type : RobotType.values()) {
      ...
      if (dp != null) {
        ...
        if (dp.alias == RobotAlias.UNKNOWN)
          ...
          this.drawString(dp.alias.toString(), (int) dp.location.x + 10, (int) dp.location.y + 10);
