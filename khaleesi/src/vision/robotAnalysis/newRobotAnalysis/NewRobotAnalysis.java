...
public class NewRobotAnalysis extends RobotAnalysisBase {
  ...
  public void nextUndistortedSpots(HashMap<SDPColor, ArrayList<Spot>> spots, long time) {
    ...
    if (lastKnownWorld != null
        && world.robotChangeDelay != 0
        && lastKnownWorld.getProbableBallHolder() != null) {
          ...
          if (ball == null) {
            ...
            for (Robot robot : world.getRobots()) {
              if (closest == null) closest = robot;
                ...
                if (lastKnownBall.location.distance(closest.location)
                    > lastKnownBall.location.distance(robot.location)) {
                  closest = robot;
                }
              }
            }
    ...
    if (lastKnownWorld != null && world.getBall() != null && lastKnownWorld.getBall() != null) {
      VectorGeometry velocity =
          VectorGeometry.fromTo(lastKnownWorld.getBall().location, world.getBall().location);
      ...
      for (RobotType rt : RobotType.values()) {
        ...
        if (newR != null) {
          if (old != null) {
            ...
            newR.velocity.direction =
                (newR.location.direction - old.location.direction) / timeDelta;
