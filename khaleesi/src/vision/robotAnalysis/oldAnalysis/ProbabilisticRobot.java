...
public class ProbabilisticRobot {
  ...
  public Robot toRobot() {
    ...
    r.location =
        new DirectedPoint(
            (int) this.centerSpots.getVector().x,
            (int) this.centerSpots.getVector().y,
            this.getHeading());
    return r;
  }
  ...
  public double getHeading() {
    ...
    if (morePink) {
      if (this.greenSpots.hasSpots())
        return greenDir.angle() - (Math.PI - MAGIC_ANGLE_NUMBER_PLEASE_CHANGE_ME);
      return pinkDir.angle() + MAGIC_ANGLE_NUMBER_PLEASE_CHANGE_ME;
    }
    if (this.pinkSpots.hasSpots())
      return pinkDir.angle() - (Math.PI - MAGIC_ANGLE_NUMBER_PLEASE_CHANGE_ME);
    return greenDir.angle() + MAGIC_ANGLE_NUMBER_PLEASE_CHANGE_ME;
  }

  public double getProbability(RobotType type, double distance) {
    if (team != null) {
      if (team == RobotColorSettings.FRIEND_COLOR
          && (type == RobotType.FOE_1 || type == RobotType.FOE_2)) return 0;
      if (team == RobotColorSettings.FOE_COLOR
          && (type == RobotType.FRIEND_1 || type == RobotType.FRIEND_2)) return 0;
    }
    if ((this.greenSpots.getSpotCount() == 0 || this.pinkSpots.getSpotCount() == 0) && team == null)
      return 0;
    if (this.greenSpots.getSpotCount() + this.pinkSpots.getSpotCount() < 2 && team == null)
      return 0;
    ...
    if (team == null
        && this.greenSpots.getMagnitudeSum() == 0
        && this.pinkSpots.getMagnitudeSum() == 0) return 0;
    int teamBonus = 0;
    if ((team == RobotColorSettings.FRIEND_COLOR)
        && (type == RobotType.FRIEND_1 || type == RobotType.FRIEND_2)) teamBonus = teamBonus + 40;
    if ((team == RobotColorSettings.FOE_COLOR)
        && (type == RobotType.FOE_1 || type == RobotType.FOE_2)) teamBonus = teamBonus + 40;
    return ((double)
            (this.pinkSpots.getMagnitudeSum() + this.greenSpots.getMagnitudeSum() + teamBonus))
        / distance;
  }
}
